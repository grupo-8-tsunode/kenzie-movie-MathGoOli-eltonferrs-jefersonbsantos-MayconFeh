    ...
    <UserReviewStyled>
      ...
      {typeof UserReview !== "undefined" && UserReview?.length > 0 ? (
       <> <p>{UserReview[0].description}</p>
        ...
        <button value={UserReview[0].id} onClick={event=>(idReviews(event.currentTarget.value),setIsEditModal(true))}> <BsPencil class="buttonIcon" size={30}/></button>
        <button value={UserReview[0].id} onClick={event=>userDeleteReview(""+event.currentTarget.value)}> <FiTrash2 class="buttonIcon" size={30} /></button>
      </>
      ) : (
